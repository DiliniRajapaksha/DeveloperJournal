...
##### Let's look at an example of a class under test which has a dependecy on `CustomerDao`
...
## HOW TO **MOCK** BEHAVIOR WITH MOCKITO
...
- Following are some of the methods that we can call on this `stub`
    ...
    - `thenThrow(exception)`
    - `thenCallRealMethod()`
    - `thenAnswer()` - this could be used when we need to mock a void method ([see How to mock void method behavior](#void)).
...
## <a name="void"></a>HOW TO **MOCK** BEHAVIOR OF VOID METHOS WITH MOCKITO
...
## HOW TO **VERIFY** BEHAVIOR WITH MOCKITO
