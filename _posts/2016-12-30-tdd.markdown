...
- **Method under test** - The target method a unit test is executing (testing). There should be no more than one method under test for a single unit test method.
- **Test fixture** - a test fixture is data that is required to execute a test. eg: an object setup in a certain state. A single test fixture could be used in several test methods.
...
I hope you found some useful information in this post. The next will be on Mockito and how to make best use of the features in Test Driven Development.
